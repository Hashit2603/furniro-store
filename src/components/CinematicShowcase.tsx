...
const SHOWCASE_PRODUCTS = [
  {
    ...
    price: "₹ 45,000",
    image: "/images/generated/sofa_3_1787452854096.jpg" // One of the premium generated ones
  },
  {
    id: 2,
    name: "Master Bed",
...
export default function CinematicShowcase() {
  ...
  return (
    <section ref={sectionRef} className="h-screen w-full bg-stone-900 text-white overflow-hidden flex items-center relative">
      ...
      <div ref={scrollWrapperRef} className="flex h-full w-[300vw]">
        {SHOWCASE_PRODUCTS.map((product, i) => (
          <div key={i} className="showcase-item w-screen h-full flex items-center justify-center p-8 relative">
            <div className="max-w-6xl w-full grid md:grid-cols-2 gap-12 items-center">
              
              <div className="order-2 md:order-1 flex flex-col items-start px-4 md:px-12">
                <h3 className="text-5xl md:text-7xl font-serif font-bold mb-6 leading-tight">{product.name}</h3>
                <p className="text-stone-400 text-xl md:text-2xl font-light mb-8 max-w-md">
                  {product.tagline}
                </p>
                <div className="text-3xl font-bold tracking-tight mb-12 text-stone-200">
                  {product.price}
                </div>
              ...
              <div className="order-1 md:order-2 h-[50vh] md:h-[70vh] w-full relative rounded-2xl overflow-hidden shadow-2xl">
                <img 
                  src={product.image} 
                  alt={product.name} 
